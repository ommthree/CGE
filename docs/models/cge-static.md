...
- **Implements:** `cge.engines.cge_static` (`CGEStaticEngine`, v0.9.13)
...
### 4f. Energy nest: KL-E-M production (Phase 5d.5 — all variants)
...
fossil carbon price contracts fossil output and expands electricity).

**Multi-region variant.** One energy nest **per region**, each calibrated over that region's own
composite intermediate flows, so a regional carbon price shifts substitution within *that region's*
