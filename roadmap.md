...
| # | Task | Effort |
|---|---|---|
| 7.1 ✅ (all variants) | **Recursive-dynamic wrapper — BUILT 2026-08-15** (`cge.dynamics`, model doc `docs/models/recursive-dynamics.md`, `dynamics` validation suite, `tests/test_dynamics.py`). Solves the `general_equilibrium` engine year-by-year, carrying the capital stock forward via 5d.3's `capital_next` (perpetual inventory + optional premature retirement / stranded assets), with exogenous labour (demographics) and Hicks-neutral productivity trends applied via the engine's new `factor_endowment_scale` hook. No perfect foresight — bookkeeping between solves. The CGE manifest now reports the benchmark stock K0 and implied growth g=INV0/K0−δ (`capital_dynamics`), so a contracting benchmark is visible. **Scope: all three CGE variants.** The closed/gov SAM and the open economy (Armington/CET + rest-of-world) carry one aggregate capital stock; **the multi-region CGE carries a per-region capital path** (each region's stock steps by its own investment, and the `factor_endowment_scale` hook moves each region's capital independently). Dynamic-capable SAMs ship for each: `toy_cge_gov`, `toy_cge_open_gov`, `toy_cge_multi_gov`. Labour/TFP trends are applied uniformly across regions (region-specific trajectories are a 7b.2 follow-up). Magnitudes illustrative (toy calibration). This is the backbone Phase 7.2 (NGFS) and 7.3 (FaIR) build on. | 4–8 wk |
...
| 7b.2 ✅ | **Structural trajectories.** Documented, sourced paths for structural change (sectoral GDP-share drift), productivity growth, population, labour-force participation, and emissions intensity — the exogenous drivers 7.1's recursive wrapper needs between static solves, replacing ad hoc "flat trend" assumptions. **DELIVERED 2026-08-16 (all four driver groups):** a `StructuralTrajectory` contract (per-year annual growth rates on a per-region and a per-sector axis, each with per-entry citation + confidence, validated on load like `ElasticitySet`), a vendored real-sourced artifact (`data/structural/trajectories_v1.json` — UN WPP 2024 population, ILO/World Bank participation, PWT 10.01 productivity, EU KLEMS sectoral productivity, IEA WEO/NGFS emissions intensity; see `data/structural/NOTICE.md`), a loader (`cge.data.structural`), and `DynamicConfig.structural` wiring. The wrapper compounds the sourced rates over the solve-year gaps: **per-region** labour-supply (population × participation) + TFP as endowment scales; **per-sector productivity** drift as a labour-augmenting composition lever (a `ProductivityShock(mechanism="labour_augmenting")` on the sector's labour input, growth-accounting-identified — see below) so the output mix shifts *endogenously* (GDP-share drift as a model result, not an imposed target); **per-sector emissions intensity** via a price-independent `emissions_intensity_scale` engine hook so a decarbonising sector faces a smaller priced wedge AND (via a base-year covered-emissions reference the wrapper feeds the engine) shows falling covered emissions vs the base year. Manifest stamps the trajectory provenance + per-entry sources; `dynamics` validation suite gains per-region-divergence, emissions-decarbonisation, and per-sector-drift checks. Flat scalars remain the fallback. Region-/sector-specific structural detail on a real EXIOBASE build is delivered by a **GDP-weighted, provenance-carrying concordance** (`data/structural/concordance_v2.json` — country-level World Bank income archetypes blended per coarse block by documented GDP weights, ISIC/EU KLEMS goods-vs-services split for sectors; `concordance_v1.json` retained for back-compat) that `structural_trajectories_for_build(regions, sectors)` applies to bind each real coarse-v3 build label to its archetype trajectory, so a real build has genuine country differentiation (advanced regions grow TFP slower than emerging; mixed regional blocks sit *between* the pure N/S paths) and sectoral composition drift instead of every label collapsing to `__all__`; an unmapped label fails loudly, `run_recursive` **rejects** an all-`__all__` run unless `allow_uniform_fallback=True`, and the mapped trajectory carries a composite concordance+trajectory provenance validated on load (review P1/P2 2026-08-29). A **documented weighting caveat** (review P1 2026-08-31, stamped into the mapped provenance + `NOTICE.md`): the same static GDP-share block weights drive population/participation/productivity alike (not per-driver weights), are held fixed over the horizon, and the residual `W*` blocks (esp. `RoW_MiddleEast`=100% S) are coarse single-archetype aggregates — per-driver, time-varying weights are the follow-up. The `sector_productivity` series is applied as a **labour-augmenting** term on the sector's labour input (`ProductivityShock(mechanism="labour_augmenting")`; VA cost falls by φ^{−s_L} under Cobb-Douglas, exact CES computed by the engine), with an **aggregate-neutral zero-mean** relative drift (weight = sector's share of value added × its labour share, `v_i·s_{L,i}`, which zeros the aggregate CD log-cost effect Σ v_i s_{L,i} ln φ_i) so a large sector's drift dominates and no aggregate level is re-imposed (review P1 2026-08-31 fixed the weight to v_i; review P1 2026-09-03 added the missing s_{L,i}). The technology term is recovered by a **two-stage source-share identification** (review P1 decomposition 2026-09-01; finite-change/log-space 2026-09-05; source-share + CES-honesty review P1 2026-09-06): **(1)** identify MFP at SOURCE — a sourced `mfp` series, or net capital deepening out of observed labour productivity using the source-period share `source_labour_shares` s_L^src (`g_MFP=g_{Y/L}−(1−s_L^src)·g_{K/L}`), NOT the receiving model's benchmark; **(2)** apply MFP through the sector's ACTUAL nest, ROUTED BY NEST TYPE and globally correct at all prices — Cobb-Douglas via labour augmentation `ln φ=ln(1+g_MFP)/s_L` (price-independent), CES via a **value-added Hicks-neutral engine channel** (`ProductivityShock(mechanism="va_hicks_neutral")`, A_va=1+g_MFP scaling the whole VA aggregate so the VA cost falls by 1/A_va at every price — a genuinely identified VA technology term, not a benchmark-only surrogate; review P1 2026-09-06). The manifest records the fine-grained mode per region-sector (`sourced_mfp` / `derived_source_share` = source-identified; `model_share_approx` = LP+deepening with only the model share; `raw_lp_heuristic`), never over-claiming "identified", and `source_labour_shares` are part of the run's content hashes. A committed EU KLEMS Törnqvist extraction is the remaining consulting-grade follow-up (see `docs/structural-data-pipeline-plan.md`). The emissions-intensity driver is a **price-independent engine hook** (`emissions_intensity_scale`) that works on real IO/satellite builds where no `carbon_cost_share` is supplied (review P1a). | 1–2 wk |
