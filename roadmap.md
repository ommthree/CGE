...
| # | Task | Effort |
|---|---|---|
| 7.1 ✅ (single capital-region) | **Recursive-dynamic wrapper — BUILT 2026-08-15** (`cge.dynamics`, model doc `docs/models/recursive-dynamics.md`, `dynamics` validation suite, `tests/test_dynamics.py`). Solves the `general_equilibrium` engine year-by-year, carrying the capital stock forward via 5d.3's `capital_next` (perpetual inventory + optional premature retirement / stranded assets), with exogenous labour (demographics) and Hicks-neutral productivity trends applied via the engine's new `factor_endowment_scale` hook. No perfect foresight — bookkeeping between solves. The CGE manifest now reports the benchmark stock K0 and implied growth g=INV0/K0−δ (`capital_dynamics`), so a contracting benchmark is visible. **Scope: any single capital-region variant — closed/gov AND the open economy** (Armington/CET + rest-of-world; both carry one aggregate capital stock, matching 5d.3). The `factor_endowment_scale` hook now works across the closed/open/multi engines and a dynamic-capable open SAM (`toy_cge_open_gov`) ships. **Multi-region capital accumulation (a per-region capital path) is the remaining follow-up.** Magnitudes illustrative (toy calibration). This is the backbone Phase 7.2 (NGFS) and 7.3 (FaIR) build on. | 4–8 wk |
