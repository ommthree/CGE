...
Engine 2 (`partial_eq` v0.4.0) as a supply-side output hit, and Engine 3 (`cge_static` v0.10.0,
**closed** variant) as a Hicks-neutral productivity multiplier θ that the general-equilibrium solve
responds to (degraded sector's price rises / output falls, factor + relative-price reallocation
economy-wide; θ=1 byte-identical, so replication/homogeneity/Walras hold). A `NatureStress` scenario
now runs end-to-end to a schema-valid `ResultSet` in PE and GE. **Remaining: 6.5** (GUI heatmaps /
drill-down / nature-run) and GE-mode consumption in the CGE's **open/multi** variants (they reject a
`ProductivityShock` for now rather than silently ignore it).
...
| 6.4 ✅ | `NatureStress` shocks: degradation scenario → productivity shocks per sector/region scaled by dependency scores → fed to the engines through the standard shock vocabulary (consumed by Engine 2 and by the **closed** Engine-3 CGE as a Hicks-neutral θ; open/multi GE consumption is a follow-up); start from published scenario sets (NGFS nature scenarios, World Bank/PIK) | 1–2 wk |
