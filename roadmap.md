...
| 7b.2 🟡 (demographic + productivity done) | **Structural trajectories.** Documented, sourced paths for structural change (sectoral GDP-share drift), productivity growth, population, labour-force participation, and emissions intensity — the exogenous drivers 7.1's recursive wrapper needs between static solves, replacing ad hoc "flat trend" assumptions. **DELIVERED 2026-08-16 (demographic + productivity):** a `StructuralTrajectory` contract (per-region, per-year annual growth rates with per-entry citation + confidence, validated on load like `ElasticitySet`), a vendored real-sourced artifact (`data/structural/trajectories_v1.json` — UN WPP 2024 population, ILO/World Bank participation, Penn World Table 10.01 productivity; see `data/structural/NOTICE.md`), a loader (`cge.data.structural`), and `DynamicConfig.structural` wiring so the wrapper compounds sourced per-region labour-supply (population × participation) and TFP paths over the solve-year gaps, per-region, replacing the flat scalars (still the fallback). Manifest stamps the trajectory provenance; `dynamics` validation suite gains a per-region-divergence check. **Remaining:** sectoral GDP-share drift (needs a solve-side mechanism, not an endowment scale) and emissions-intensity trajectories (belongs with the 5d.5/7b.5 carbon/energy machinery, double-count-checked). | 1–2 wk |
